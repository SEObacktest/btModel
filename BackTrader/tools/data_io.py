...
import numpy as np
import BackTrader.tools.log_func as log
import BackTrader.config
...
class DataIO():
    ...
    def input_futureInformation():
        ...
        """
        # 显示股票信息字典
        name_dict = show_future_codes_from_mysql()  # 从MySQL数据库中读取所有上市交易的期货代码，并返回期货名称、保证金比例、合约乘数以及文化码的对应字典。
        ...
        while True:
            ...
            if name in name_dict:
                names.append(name)  # 将用户输入的期货名称添加到名称列表中
            else:
                print("输入期货不存在，请重新输入")
                continue  # 如果股票名称不存在，提示重新输入
        # print(names)
        ...
        for name in name_dict:
            ...
            # #if 1:
            wh_codes.append(name_dict[name]['wh_code'])  # 将用户输入期货的文华码添加到列表中
            margins.append(name_dict[name]['margin'])  # 将用户输入期货的保证金比例称添加到列表中
            mults.append(name_dict[name]['mult'])  # 将用户输入期货的保证金乘数称添加到列表中

        # 选回测周期
        # 创建一个字典里面有用户的选项和对应的tuple
        ...
        if period_input not in period_options:
            ...
            exit()
        # period等于客户选择的period的tuple的第一个值，has_time等于客户选择的period的tuple的第二个值
        period, has_time = period_options[period_input]

        # 检测合法输入时间的功能，将prompt跟has_time（day,min,s）应有的数字位数对比，也测试时间是否晚于现在时间
        ...
        #让用户输入回测起始和结束时间
        while True:
            # 输入开始时间并让get_valid_date检测是否合规
            start_date = get_valid_date(
                "请按说明格式输入回测起始日期或时间：\n（例如：若为2024年12月16日15点30分15秒则应输入：20241216153015）\n",
                has_time)
            # 输入结束时间并让get_valid_date检测是否合规
            ...
            if end_date >= start_date:
                break
        #对输入时间检测合规后给DataIO的开始结束时间赋值，并返回股票代码列表和日期
...
def show_future_codes_from_mysql():
    ...
    """
    connection = get_engine()  # 获取服务器地址

    # 使用pandas读取MySQL数据库中的表 future_codes
    query = "SELECT 期货名, 保证金比例, 合约乘数, 别名, wh_code FROM future_codes"  # 设一个sql query语句：获取数据库future_codes表中的SELECT之后的那些列
    data = pd.read_sql(query, con=connection)  # 用设定的sql query语句读取数据库，并返回一个Dataframe

    # 打印期货品种代码列表
    print("===================期货品种代码列表===================")
    # 使用上下文管理器pd.option_context临时更改 Pandas 的显示选项
    # 'display.max_columns', None: 设置为显示所有列（默认可能会省略部分列）
    # 'display.max_rows', None: 设置为显示所有行（默认可能会省略部分行）
    # 此设置仅在 with 语句块内生效，代码块结束后会自动恢复原始显示设置
    with pd.option_context('display.max_columns', None, 'display.max_rows', None):
        print(data.iloc[:, :4])  # 打印Dataframe的前4列
    print("===============================================")
    ...
    name_dict = {
        ...
        # 遍历 Pandas 数据框中的每一行
        # for _: 表示忽略索引值，仅使用行数据
        # row: 表示每次迭代时获取的当前行数据，类型是 Pandas 的 Series
        # data.iterrows(): 以 (索引, 行数据) 的形式生成数据框的迭代器
