...
class Buy_And_Sell_Strategy(bt.Strategy):
    ...
    def buy_short_function(self, line, size):
        """
        ...
        if buy_short_cond and not self.position and self.broker.getcash() > 0:
            self.log(f'BUY LONG CREATE, {line._name}, Size: {size}, Price: {line.close[0]:.2f}')
            self.buy(data=line, size=size)
        else:
            pass

    def sell_short_function(self, line, size):
        """
        ...
        if sell_short_cond and self.broker.getcash() > 0:  # 确保资金充足，则执行卖空操作
            self.log(f'SELL SHORT CREATE, {line._name}, Size: {size}, Price: {line.close[0]:.2f}')
