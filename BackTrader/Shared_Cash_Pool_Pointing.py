...
class Shared_Cash_Pool_Pointing(bt.Strategy):
            ...
            for data2 in self.datas:
                ...
                if self.is_trade[data2]==False:#如果找到了今天没有交易的品种
                    if position_size>0:#如果持多仓
                        ...
                        self.info.loc[self.num_of_all]=[current_date,future_name,"不交易",0,0,0,0,round(self.cash),round(self.average_open_cost[data2]),round(self.paper_profit[data2]),round(total_value),data2.close[0],round(total_margin),None]
                        ...
                        total_value+=self.cash
                        self.info.loc[self.num_of_all]=[current_date,future_name,"不交易",0,0,0,0,round(self.cash),round(self.average_open_cost[data2]),round(self.paper_profit[data2]),round(total_value),close_value,round(total_margin),None]
                        self.num_of_all+=1#临时充当dataframe的行数
                        #注意到这其实影响了真正的交易次数
                    #就算不交易，也需要重新计算保证金
    ...
    def next(self):
        ...
        if self.params.backtest_start_date <= current_date <= self.params.backtest_end_date:
                ...
                if self.is_trade[data2] == False:  # 如果找到了今天没有交易的品种
                    if positon_size > 0:  # 如果持多仓
                        ...
                        self.info.loc[self.num_of_all] = [current_date, future_name, "不交易", 0, 0, 0, 0,
                                                          ...
                                                          round(self.paper_profit[data2]), round(total_value),
            ...
            self.info.loc[self.num_of_all] = [current_date, "ALL", None, 0, 0, 0, 0, round(self.cash), None, None,
                                              round(total_value), None, round(total_margin), None]
    ...
    def stop(self):
        ...
        for data in self.datas:#遍历所有品种
            ...
            if position_size!=0:#如果有持仓
                self.profit[data._name]+=position_size_abs*abs(close_value)
                #调整利润
    ...
    def notify_order(self,order):
        ...
        self.info.loc[self.num_of_all]=[current_time,future_name,trade_type,unit_price,trade_nums,round(trade_value),round(trade_comm),round(self.cash),round(self.average_open_cost[data]),round(self.paper_profit[data]),round(total_value),close_value,round(total_margin),close_profit]
